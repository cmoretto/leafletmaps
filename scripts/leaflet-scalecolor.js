(function($, L) {
    ...
    _self.init = function (min, max, options) {
        _self.min = parseFloat(min);
        _self.max = parseFloat(max);
        _self.range = max - min;
    ...
    _self.getNewColor = function (val) {
        if (val > _self.max) {
            val = _self.max;
        }
        ...
        var c = _self.settings.nullColor;
        if (val !== null && !isNaN(val) && _self.range !== 0) {
            var adj = val - _self.min;
            var pct = 1.0 * adj / _self.range;
            c = getColor(pct);
        }
    ...
    var getColor = function (percent) {
        ...
        var colors = _self.settings.colorArray.length;

        var colorPosition = percent * (colors - 1);
        var sIndex = Math.floor(colorPosition);
        sIndex = Math.min(colors - 2, sIndex);

        var s = _self.settings.colorArray[sIndex];
        var e = _self.settings.colorArray[sIndex + 1];
    ...
    function transition3(value, maximum, s, e) {
        ...
        value = transition(value, maximum, s[2], e[2]); // v
        return [hue, saturation, value];
    }


    // aggiunge il tipo ScaleColor a leaflet.js.
    L.ScaleColor = L.Class.extend({
        getColor: _self.getNewColor,

        initialize: function (min, max, options) {
            _self.init(min, max, options);
        }
    });

    // crea una factory al tipo ScaleColor in leaflet.js.
    L.scaleColor = function (min, max, options) {
        return new L.ScaleColor(min, max, options);
    };
